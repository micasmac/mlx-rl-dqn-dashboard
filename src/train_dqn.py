...
class DQNTrainingWrapper:
    ...
    def train(self, env_name=None, total_timesteps=None, output_dir='docs/results'):
        ...
        try:
            ...
            progress_bar = tqdm(range(self.args.total_timesteps), desc="Training DQN")
            ...
            for global_step in progress_bar:
                ...
                if random.random() < epsilon:
                    ...
                    q_values = agent.q_network(mx.array(obs.reshape(1, -1)))
                ...
                # Convert the action to a NumPy array before adding to the buffer
                # Stable-baseline3's replay buffer expects NumPy arrays not int
                action_np = np.array(action, dtype=np.int64)  # Use np.int64 for compatibility

                # Store transition in replay buffer
                replay_buffer.add(obs, next_obs, action_np, reward, done, info)  # <-- Pass the NumPy array here
                ...
                if done:
                    ...
                    progress_bar.set_postfix({
                        'reward': f"{current_episode_reward:.1f}",
            ...
            import traceback
            traceback.print_exc()
            # Only use dummy data if we have absolutely no real data
            if not self.training_history['episode_rewards']:
                print("Generating dummy training data for demonstration...")
